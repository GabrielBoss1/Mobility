...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      em("Positive and Negative percentanges indicate an increase and decrease from the aseline period(median value between January 3 and february 6, 2020) respectively"),
...
server <- function(input, output) {
  ...
  output$download_data <- downloadHandler(
    ...
    content = function(file) {
      data <- filtered_data()
      write.csv(data_file_row.names = FALSE)
    }
  )
}
